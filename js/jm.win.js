    ...
    **/
    this.__initBody = function () {
        ...
        if (this.title) {
            ...
            //功能区域            
            //添加关闭按钮
            var closebtn = $("<a class='cmdbtn close' onmousedown='javascript:event.stopPropagation();' onclick='javascript:$jm.closeWinById(\"" + this.id() + "\");return false' href='javascript:;'>X</a>");
            closebtn.appendTo(this.winHead);

            if (this.resizeable && this.canMax) {
                //最大化
                var maxbtn = $("<a class='cmdbtn max' onmousedown='javascript:event.stopPropagation();' onclick='javascript:$jm.getWinById(\"" + this.id() + "\").changeState();return false' href='javascript:;'>口</a>");
                maxbtn.appendTo(this.winHead);
            }

            if (this.canMin) {
                //最小化
                var minbtn = $('<a class="cmdbtn min" onmousedown="javascript:event.stopPropagation();" onclick="javascript:$jm.getWinById(\'' + this.id() + '\').min();return false;">一</a>');
